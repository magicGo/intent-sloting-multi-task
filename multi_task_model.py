...
class MultiTaskModel(object):
    def __init__(self,
        ...
        self.best_dev_accuracy = tf.Variable(0.0, trainable=False)
        self.best_test_accuracy = tf.Variable(0.0, trainable=False)
        ...
        for i in xrange(buckets[-1][0]):
            setattr(self, 'encoder_input_%d' % i, tf.placeholder(tf.int32, shape=[None], name="encoder{0}".format(i)))
            self.encoder_inputs.append(getattr(self, 'encoder_input_%d' % i))
        for i in xrange(buckets[-1][1]):
            ...
            setattr(self, 'tag_weight_%d' % i, tf.placeholder(tf.float32, shape=[None], name="weight{0}".format(i)))
            self.tag_weights.append(getattr(self, 'tag_weight_%d' % i))
    ...
    def joint_step(self, session, encoder_inputs, tags, tag_weights,
        ...
        self.input_feed = {}
        self.input_feed[self.sequence_length.name] = batch_sequence_length
        for l in xrange(encoder_size):
            self.input_feed[self.encoder_inputs[l].name] = encoder_inputs[l]
            self.input_feed[self.tags[l].name] = tags[l]
            self.input_feed[self.tag_weights[l].name] = tag_weights[l]
        self.input_feed[self.labels[0].name] = labels[0]
        ...
        outputs = session.run(output_feed, self.input_feed)
        if not forward_only:
