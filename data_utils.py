...
def create_vocabulary(vocabulary_path, data_path, tokenizer=None, normalize_digits=True):
    ...
    if not gfile.Exists(vocabulary_path):
        ...
        with gfile.GFile(data_path, mode="r") as f:
            ...
            for line in f:
                counter += 1
                if counter % 100000 == 0:
                    tf.logging.info("\tprocessing line %d" % counter)
...
def initialize_vocab(vocabulary_path):
    ...
    if gfile.Exists(vocabulary_path):
        ...
        rev_vocab = {index:line.strip() for index, line in enumerate(words)}
...
def prepare_multi_task_data(data_dir):
    train_path = data_dir + '/train/train'
    dev_path = data_dir + '/valid/valid'
    test_path = data_dir + '/test/test'

    # Create vocabularies of the appropriate sizes.
    in_vocab_path = os.path.join(data_dir, "in_vocab.txt")
    out_vocab_path = os.path.join(data_dir, "out_vocab.txt")
    ...
    in_seq_train_ids_path = train_path + (".ids.seq.in")
    out_seq_train_ids_path = train_path + (".ids.seq.out")
    ...
    in_seq_dev_ids_path = dev_path + (".ids.seq.in")
    out_seq_dev_ids_path = dev_path + (".ids.seq.out")
    ...
    in_seq_test_ids_path = test_path + (".ids.seq.in")
    out_seq_test_ids_path = test_path + (".ids.seq.out")
